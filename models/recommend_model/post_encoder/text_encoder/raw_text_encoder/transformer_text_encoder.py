...
class TransformerTextEncoder(BaseRawTextEncoder):
    def __init__(self, out_features: int, word_embedding: nn.Embedding, num_heads: int, query_vector_dim: int, drop_out: float):
        super(TransformerTextEncoder, self).__init__(out_features=out_features, word_embedding=word_embedding)
        self.drop_out = nn.Dropout(p=drop_out)
        self.transformer = Transformer(in_features=word_embedding.embedding_dim, out_features=out_features, num_heads=num_heads, drop_out=drop_out)
    ...
    def forward(self, token_ids: Tensor, attention_mask: Tensor = None) -> Tensor:
        ...
        word_context_matrix = self.transformer(x=word_embedding_matrix, attn_mask=attn_mask_expand)   # batch_size, sequence_len, embedding_dim
