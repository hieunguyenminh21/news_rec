...
class MultiHeadSelfAttentionUserEncoder(BaseHistoryUserEncoder):
    def __init__(self, out_features: int, num_heads: int, query_vector_dim: int):
        super(MultiHeadSelfAttentionUserEncoder, self).__init__(out_features=out_features)
        self.self_attention = MultiHeadSelfAttention(in_features=out_features, out_features=out_features, num_heads=num_heads)
    ...
    def forward(self, x: Tensor, attention_mask: Tensor = None) -> Tensor:
        ...
        matrix = self.self_attention(x=x, attn_mask=attn_mask_expand, return_full=False)   # batch_size, sequence_len, in_features
